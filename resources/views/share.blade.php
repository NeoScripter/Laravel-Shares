@auth
    <form action="{{ route('shares.store') }}" method="post" class="share">
        ...
        <textarea name="share" id="share__textarea" rows="4"
            class="{{ $errors->has('share') ? 'share__textarea--error' : '' }}"></textarea>
        @error('share')
...
@guest
    <div class="share">
        <a href="{{ route('login') }}" class="share__label">Log in to share your thoughts</a>
    </div>
@endguest
